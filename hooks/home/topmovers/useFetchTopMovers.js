	...
	useEffect(() => {
		const fetchTopMoversAndBarData = async () => {
			try {
				...
				const response = await fetch(`${REACT_APP_BASE_URL}/top-movers`);
				...
				const barsResponse = await fetch(`${REACT_APP_BASE_URL}/historical-bars?symbols=${symbols}`);
		...
		const fetchTopMoversAndBarDataCrypto = async () => {
			try {
				...
				const responseCrypto = await fetch(`${REACT_APP_BASE_URL}/top-movers-crypto`);
				...
				const barsResponseCrypto = await fetch(`${REACT_APP_BASE_URL}/historical-bars-crypto?symbols=${symbolsCrypto}`);
